...
#   - small utilities
# The MIA estimators (mia-pkg-ice, mia-tmle) are implemented in doParallel_IAI.R
#  using the miapack and tmle packages. The high-dimensional auxiliary block W is
#  generated by helper_IAI_Wblock.R.
...
run_missingness_model <- function(data, vars) {
  ...
  nchains = 3 
  # each chain's JAGS RNG is seeded from R's RNG, so results are reproducible
  #  given the R seed set in doParallel_IAI.R
...
# Template for one sbatch file; capitalized words are placeholders filled by
#  generateSbatch(). Lines must not be indented, or SLURM will silently ignore
...
# generateSbatch()
# Writes one sbatch file per row of sbatch_params by filling the placeholders in
#  sbatch_skeleton() with that row's values. Additional placeholders can be mapped to
#  columns via extra_placeholders = c(PLACEHOLDER = "column_name"). Optionally also
#  writes a runfile that submits every job.
...
# Identify sbatch jobs that did not write a results file (e.g., because they
#  exceeded their wall time). Results files are named <prefix>_job_<number>_.csv.
# .max.sbatch.num: number of sbatch files; defaults to the largest job number found.
# Writes missed_job_nums.csv to .results.write.path and returns the job numbers.
